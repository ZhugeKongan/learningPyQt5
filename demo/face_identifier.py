...
class FaceID:
    ...
    def __init__(self, face_db_path, unknown_threshold=0.0, ai85_adapter=None):
        ...
        self.subj_ids = np.array(list(range(len(subject_names_list))))#0-1

        self.set_unknown_threshold(unknown_threshold)#1.0*560.0
    ...
    def __get_sorted_mean_distances_to_subjects(self, embedding):
        dists = embedding.astype(np.float32) - np.array(self.embedding_list.astype(np.float32))#1-2*10
        dists = np.sqrt(np.sum(np.square(dists), axis=1))

        mean_dists = np.zeros((len(self.subj_ids), ), dtype=np.float32)#2

        for i, sid in enumerate(self.subj_ids):#2
            mean_dists[i] = dists[self.subj_list == sid].mean()#跟所有同类参考向量间的平均距离
    ...
    def __get_sorted_subjects(self, outputs):
        idx = np.argsort(outputs)
        outputs = outputs[idx]
        subjects = self.subj_ids[idx]
    ...
    def run(self, img):
        ...
        if self.ai85_adapter:
            ...
            t_get_emb = time.time()#片上推理时间
            # 计算距离，并返回有序距离，及其对应的类名
