...
def cvt_qimage_to_grayimg(q_img, share_memory=False):
    """ Creates a numpy array from a QImage.
    ...
    """
    assert isinstance(q_img, QImage), "img must be a QtGui.QImage object"
    assert q_img.format() == QImage.Format_Grayscale8, "img format must be QImage.Format.Format_RGB32, got: {}".format(q_img.format()) #pylint: disable=line-too-long

    img_size = q_img.size()
    print(img_size,q_img.depth())
    buffer = q_img.constBits()
    buffer.setsize(img_size.height() * img_size.width() * 2)

    # Sanity check
    n_bits_buffer = len(buffer) * 4
    n_bits_image = img_size.width() * img_size.height() * q_img.depth()
    assert n_bits_buffer == n_bits_image, \
        "size mismatch: {} != {}".format(n_bits_buffer, n_bits_image)

    assert q_img.depth() == 8, "unexpected image depth: {}".format(q_img.depth())
